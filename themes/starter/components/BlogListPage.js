...
const BlogListPage = ({ page = 1, posts = [], postCount, siteInfo }) => {
  ...
  useEffect(() => {
    ...
    setFilterPosts(newFilterPosts.slice(0,3));
  }, [columns, posts]);
  ...
  if (!filterPosts || filterPosts.length === 0) {
    ...
    return (
      <div>
        {/* 文章列表 */}
        <div id='posts-wrapper' className='container-news'>
          {filterPosts?.map((post, index) => (
            <div
              key={post.id}
              className={`justify-center ${index === 0 ? `main-news-card bg-black` : `side-news-card bg-amber-300 col-span-1 row-span-1`}`}
